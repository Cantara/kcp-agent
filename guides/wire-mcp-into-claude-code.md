...
| `kcp_replay` | the cross-examination of a saved `kcp_plan` artifact — manifest bytes re-hashed, plan re-run from the echoed inputs, **identical** or **drifted** per manifest |
...
`follow`, `max_depth` — and the CLI's full capability surface: `role`, `methods` (payment
methods, e.g. `["free","x402"]`), `credentials` (e.g. `["mtls"]`), `attest` (attestation
provider). Attestation and credential gates answer over MCP exactly as on the command line;
see the *Borrowed Leash* demo (`node examples/demos.js leash`).
